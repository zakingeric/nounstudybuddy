...
async function generateSmartPodcast() {
  ...
  try {
    ...
    if (data.choices && data.choices[0] && data.choices[0].message) {
      scriptArea.value = data.choices[0].message.content;
      processText.textContent = 'Podcast Generated!';
      processBar.style.width = '100%';
    } else {
      scriptArea.value = '❌ No valid response received from AI.';
      processText.textContent = 'Error: No output from AI';
    }

  } catch (error) {
    console.error('Podcast error:', error);
    scriptArea.value = '❌ Error generating podcast: ' + error.message;
